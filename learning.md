...
2. **Create a Render "Web Service" from this repo, branch `release`.**
   ...
   - **"Root Directory," briefly:** for monorepos — narrows *both*
     building and auto-deploy-triggering to one subfolder, independent
     of the branch filter (branch = which history; this = which
     subfolder of it). That folder only ever needs a `Dockerfile`, never
     a `docker-compose.yml`. Not a monorepo here — **leave it blank.**

   **How do Django and Redis reach each other without Compose, if
   they're not guaranteed to be on the same machine?** They aren't, and
   don't need to be — Render gives every service in the same account +
   region a shared **private network**, regardless of which physical
   machine each actually runs on. Any two of your own services there
   (web, Redis, etc.) get a stable internal hostname
   (`<service-name>-<hash>:<port>`) and can reach each other directly
   over it, traffic never leaving Render's infrastructure — you just
   paste that internal address into `REDIS_HOST` as an env var (step 3).
   Same *outcome* as Compose (private, name-based reachability) via a
   different *mechanism* — a cloud-wide private network instead of one
   Docker bridge on one machine. The one hard rule: same account **and**
   same region — a service in one region can't reach another region's
   service this way at all, only over the public internet instead.

   Sources: [Render private networking](https://render.com/articles/how-render-handles-private-networking), [Render Private Network docs](https://render.com/docs/private-network)
...
4. **Set the rest of the env vars in Render's dashboard** —
   ...
   - **Why `SECRET_KEY` matters, even though the rate limiter itself
     never touches it:** `config/urls.py` still wires up `admin/` —
     Django's built-in, session-based admin login, generated by
     `startproject` and never removed. `SessionMiddleware`/
     `CsrfViewMiddleware` are also still active in `MIDDLEWARE`. So this
     isn't hypothetical future-proofing: the moment anyone logs into
     `/admin/`, or this app grows any login/form, Django signs that with
     `SECRET_KEY` right now. Leak it, and anyone can forge a valid
     session for any user, including an admin. That's why it's an env
     var, not hardcoded: a
     committed file is readable by anyone with repo access (and
     actively scanned for by bots), while an env var can differ per
     environment and be rotated instantly with no code change and no
     trace left in git history. The current `.env` value is literally
     Django's own `startproject` default (note the `django-insecure-`
     prefix — that's Django's own warning label) — fine for local dev,
     but generate a genuinely random one for Render's dashboard.
   - **`.env`/env vars vs. "secret files" — the main difference:** an
     env var lives in the process's ambient environment, which leaks
     more easily than it feels like it should — inherited by every
     child process, visible in plaintext via `docker inspect`, and
     prone to ending up in logs or crash-dump pages that naively print
     "the environment" for debugging. A **secret file** (Docker Swarm
     secrets, Kubernetes Secrets) instead writes the value to a file at
     a fixed path (e.g. `/run/secrets/secret_key`), usually on an
     in-memory filesystem — your app reads it as a file, not
     `os.environ`, so it's never inherited by child processes and never
     shows up in an env-var dump; the orchestrator also encrypts it at
     rest. More setup, and needs Swarm/Kubernetes-level support — Render
     (like most PaaS platforms) only offers the env-var model, which is
    ...
    services:
      redis:
        ...
        ports:
          - 6379:6379
```
Simplest way to picture the whole job: GitHub hands you a **brand new,
completely blank rented computer** for a few minutes, then deletes it
when the job ends. Blank means *actually* blank — no Redis, no your
code, nothing pre-installed.

`services:` is just you telling GitHub: "before my steps start, please
also switch on a Redis container on this same rented computer, running
in the background." Without it, when a later step tries to talk to
`localhost:6379`, nothing would be listening there at all.

**Does `ports: 6379:6379` here expose Redis to the public internet?** No
— unlike the earlier Networking lessons, "the host" in this case is the
rented computer itself, and *that* has no public IP anyone could reach
in the first place; it's destroyed minutes later. Publishing a port only
ever means "reachable at `localhost:6379` by other steps on this same
temporary machine" — the mechanism is identical to before, but there's
no public exposure because the underlying machine was never reachable
from outside to begin with.
...
```
Same idea: that rented computer has none of your project's files on it
yet — not even the `Dockerfile`. This step's whole job is to copy your
GitHub repo onto it. Skip it, and the very next step
(`docker build .`) would fail immediately — there'd be nothing there to
build.
